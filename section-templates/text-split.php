...
<a id="<?php echo get_field('anchor_id'); ?>"></a>

<div id="<?php echo esc_attr($id); ?>" class="<?php echo esc_attr($className); ?>" style="background-color: var(--<?php echo get_field('background_colour'); ?>);">
    ...
    <div class="uk-container p-t-<?php echo esc_html(get_field('padding_top')); ?> p-b-<?php echo esc_html(get_field('padding_bottom')); ?>">
        ...
        <div class="text-split-container">
            <div class="text-split-title animatable fadeInDown animationDelayone">
                <?php echo '<h2 class="modTitle">' . get_field('text_split_title') . '</h2>'; ?>
                <?php
                $text_split_intro = get_field('text_split_introduction');
                if (!empty($text_split_intro)) {
                    echo '<p class="subPara">' . esc_html($text_split_intro) . '</p>';
                }
                ?>
                <div class="text-split-button">
                    ...
                    if (have_rows('text_split_buttons')) :
                        while (have_rows('text_split_buttons')) : the_row();
                            ...
                            $is_popup = get_sub_field('text_split_link_type');
                            $button_colour = get_sub_field('text_split_button_colour');

                            if ($is_popup) :
                                $popup_text = get_sub_field('text_split_popup_text');
            ...
            <div class="text-split-text animatable fadeInDown animationDelaytwo">
                ...
                <?php $link = get_field('link_title');
                if ($link) : $link_url = $link['url'];
                    $link_title = $link['title'];
                    $link_target = $link['target'] ? $link['target'] : '_self'; ?>
                    <a class="highlight" href="<?php echo esc_url($link_url); ?>" target="<?php echo esc_attr($link_target); ?>">
                        <?php $image = get_field('logoaward');
            ...
            <div class="mobile-text-split-button">
        ...
        <div id="<?php echo esc_attr($unique_id); ?>" class="popup-box popup-box-sustainability hide">
            ...
            <div class="popup-wrapper popup-contact-wrapper">
                ...
                <div class="popup-body popup-contact-body">
                    ...
                    <?php
                    $ts_pop_subtitle = get_sub_field('ts_popup_subtitle');
                    $ts_pop_title = get_sub_field('ts_popup_title');
                    $ts_pop_content = get_sub_field('ts_popup_content');
