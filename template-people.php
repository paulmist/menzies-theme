...
/**
 * Template Name: People Page (Custom Archive)
 */
...
<head>
    <title>Menzies People</title>
</head>

<?php
$inner_page_header = get_field('people_inner_page_header', 'option');
$sector_header_type = get_field('people_header_format', 'option');
?>
...
<main id="primary" class="site-main stories">
    ...
    <?php if ($sector_header_type === 'image') : ?>
        ...
        <div class="pageHeader backgroundHeader animatable fadeInDown animationDelayone">
            ...
            <?php
            $image = $inner_page_header['people_header_background_image'];
            if (!empty($image)): ?>
                <img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />
            <?php endif; ?>

            <div class="<?php if ($inner_page_header['people_centred_text']) : echo 'pageHeaderCenter';
                        endif; ?> p-t-xlarge p-b-medium animatable fadeInDown animationDelayone">
                <h1 class="<?php if ($inner_page_header['people_centred_text']) : echo 'justifyCenter';
                            endif; ?>">

                    <?php $image = $inner_page_header['people_identitylogo'];
                    if (!empty($image)) : ?>
                        <img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />
                    <?php endif; ?>

                    <?php echo $inner_page_header['people_inner_header_title']; ?>
                </h1>
                <p class="subPara <?php if ($inner_page_header['people_centred_text']) : echo 'justifyCenter';
                                    endif; ?>"><?php echo $inner_page_header['people_inner-header-sub']; ?></p>
            </div>
        </div>
    <?php endif; ?>


    <div class="filter-yesmobile mob-filter filter-header">
        <div class="filter-header-inner">
            <h2 class="modTitle">Meet Our Experts</h2>
            <div class="ms-h-filter">
                <?php echo do_shortcode('[searchandfilter id="24957"]'); ?>
            </div>
        </div>
    </div>
    ...
    <div class="filter-yesmobile mobile-search-filter">
    <?php echo do_shortcode('[searchandfilter id="24957" fields="search" submit_button="false" reset_button="false"]'); ?>
        ...
        <div class="knowledgebase-archive-listing">
            ...
            <form class="searchandfilter" method="get">
                <?php echo do_shortcode('[searchandfilter id="24957" show="results"]'); ?>
            </form>
        </div>
    </div>
